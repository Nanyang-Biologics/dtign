...
class TrainLogger(BasicLogger):
    def __init__(self, args, config, create=True):
        self.args = args
        log_name = 'eval' if args['mark'] == 'eval' else 'train'
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        if args['start_checkpoint'] is not None:
            pattern = r"\/(\d+_.*?)\/"
            match = re.search(pattern, args['start_checkpoint'])
            savetag = match.group(1)
        else:
            if args.get('mark') == None:
                savetag = timestamp + '_' + args.get('model') + '_' + 'fold_' + str(args.get('fold'))
            else:
                savetag = timestamp + '_' + args.get('model') + '_' + 'fold_' + str(args.get('fold')) + '_' + args.get('mark')

        save_dir = args.get('save_dir')
        if save_dir == None:
            raise Exception('save_dir can not be None!')
        train_save_dir = os.path.join(save_dir, savetag)
        self.log_dir = os.path.join(train_save_dir, 'log', log_name)
